...
- **编辑区语法高亮**：CodeMirror 6，Markdown + 内嵌 HTML（`<table>` 标签上色）+ fenced 代码块（JS/Python）实时上色
- **原生 HTML 兼容**：`<tr>`、`<br>`、`<table>` 等原样透传渲染（markdown-it `html: true`）
- **数学公式**：MathJax v4，支持 `\(...\)` `$...$`（行内）与 `$$...$$` `\[...\]`（块级）
- **格式化渲染**：标题 / 表格 / 引用 / 列表 / 代码块等
- **代码上色**：highlight.js 按语言高亮（未标注语言时自动检测）
- **滚动同步**：编辑区与预览区按行联动（CodeMirror 精确行定位），可开关
- **亮 / 暗主题**：跟随系统，工具栏按钮手动切换（记忆选择）
- **草稿自动保存**：localStorage，刷新自动恢复
- **工具条**：格式化（HTML 美化）/ 自动换行 / 复制 / 粘贴替换 / 清空（二次确认）/ 主题 / 滚动同步
- **无 banner**：满视口双栏，工具条是唯一 UI

## 技术栈

- Cloudflare **Workers Assets**（`wrangler.jsonc` 配置，`not_found_handling: single-page-application`）
- 编辑器：**CodeMirror 6**（markdown + 内嵌 HTML 高亮，JS/Python 代码块支持）
- markdown-it v15（html:true + typographer + 自定义 highlight）
- MathJax v4（`mathjax@4/tex-chtml.js`，CDN 加载）
- highlight.js v11（打包进 bundle，未标注语言自动检测）
